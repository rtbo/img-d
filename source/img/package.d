/// Images IO and manipulation module.
module img;
...
enum ImageFormat
{
    ...
    /// channel. Applications can use this format to boost the alpha blending
    /// computations. Stored in machine native byte order.
    argbPremult,
}
...
/// The number of bytes needed for each row of an image to be aligned to the
/// given alignment (4 bytes by default).
/// This is often useful for image manipulation or to be used as textures.
size_t imageAlignedStride(in ImageFormat format, in uint width, in size_t alignment=4) pure
{
    immutable size_t bits = format.bpp * width;
    return (((bits + 7) / 8) + alignment-1) & (-alignment);
}

unittest
{
    enum strideAlignment = 4;
    assert(ImageFormat.a1.imageAlignedStride(0) == 0);
    assert(ImageFormat.a1.imageAlignedStride(1) == strideAlignment);
    assert(ImageFormat.a1.imageAlignedStride(5) == strideAlignment);
    assert(ImageFormat.a1.imageAlignedStride(48) == 2*strideAlignment);
    assert(ImageFormat.a1.imageAlignedStride(64) == 2*strideAlignment);
    assert(ImageFormat.a1.imageAlignedStride(65) == 3*strideAlignment);

    assert(ImageFormat.a8.imageAlignedStride(1) == strideAlignment);
    assert(ImageFormat.a8.imageAlignedStride(3) == strideAlignment);
    assert(ImageFormat.a8.imageAlignedStride(4) == strideAlignment);
    assert(ImageFormat.a8.imageAlignedStride(5) == 2*strideAlignment);
    assert(ImageFormat.a8.imageAlignedStride(8) == 2*strideAlignment);
    assert(ImageFormat.a8.imageAlignedStride(9) == 3*strideAlignment);

    assert(ImageFormat.argb.imageAlignedStride(1) == strideAlignment);
    assert(ImageFormat.argb.imageAlignedStride(2) == 2*strideAlignment);
    assert(ImageFormat.argb.imageAlignedStride(3) == 3*strideAlignment);
    assert(ImageFormat.argb.imageAlignedStride(4) == 4*strideAlignment);
    assert(ImageFormat.argb.imageAlignedStride(5) == 5*strideAlignment);
}

/// Format of a serialized image
...
class Image
{
    ...
    private uint _width; // in px
    private uint _height; // in px
    private size_t _stride; // in bytes

    /// Allocates an image with format and size.
    /// Image stride is given by max(minStride, format.imageAlignedStride(size.width))
    /// The pixel content is not initialized.
    this(ImageFormat format, uint[2] size, size_t minStride=0)
    {
        ...
        _width = size[0];
        _height = size[1];
        _stride = max(minStride, format.imageAlignedStride(size[0]));
    ...
    private immutable this (immutable(ubyte)[] data, ImageFormat format,
                            uint width, uint height, size_t stride)
    {
        _data = data;
        _format = format;
        _width = width;
    ...
    inout(ubyte)[] line(in uint l) inout
    in
    {
        assert(l < _height);
    }
    ...
    void blitFrom(const(Image) src, in int[2] srcOrig, in int[2] destOrig, in uint[2] size, in bool yReversed=false)
    {
        enforce(format == src.format, "image blit: format mismatch");
        enforce(src.width >= srcOrig[0]+size[0], "image blit: src[0] too small");
        enforce(src.height >= srcOrig[1]+size[1]);
        enforce(this.width >= destOrig[0]+size[0]);
        enforce(this.height >= destOrig[1]+size[1]);
        assert(format != ImageFormat.a1);

        immutable pixelStride = bpp(format)/8;
        immutable copyStride = pixelStride * size[0];

        foreach(l; 0 .. size[1]) {
            immutable srcL = yReversed ? (size[1]-(l+srcOrig[1])-1) : (l+srcOrig[1]);
            immutable srcPos = srcL * src.stride + pixelStride * srcOrig[0];
            immutable destL = l+destOrig[1];
            immutable destPos = destL * this.stride + pixelStride * destOrig[0];
    ...
    Image convert(in ImageFormat format) const
    {
        ...
        immutable stride = format.imageAlignedStride(_width);
        auto res = new Image (
...
Image slice(Image img, in int[2] offset, in uint[2] size)
{
    assert(img);
    assert(img.format != ImageFormat.a1);
    enforce(offset[0]+size[0] <= img.width);
    enforce(offset[1]+size[1] <= img.height);

    const bytesPerPix = img.format.bpp / 8;
    const dataOffset = (offset[1]*img.stride + offset[0]) * bytesPerPix;
    const dataLen = (size[1]*img.stride + size[0]) * bytesPerPix;

    auto dataSlice = img.data[dataOffset .. dataOffset + dataLen];
    return new Image (
        dataSlice, img.format, cast(ushort)size[0], cast(ushort)size[1], img.stride
    );
}

/// Create a slice of the given image, referencing the same data.
/// ImageFormat.a1 is not supported.
immutable(Image) slice(immutable(Image) img, in int[2] offset, in uint[2] size)
{
    assert(img);
    assert(img.format != ImageFormat.a1);
    enforce(offset[0]+size[0] <= img.width);
...
private
{
    import img.libjpeg.turbojpeg;
    import img.libpng.png;
    import std.path;
    import std.string;
    ...
    class PngIO : ImgIO
    {
        ...
        private Image readFinish(png_imagep pimg, in ImageFormat format)
        {
            ...
            immutable rowStride = imageAlignedStride(readFmt, pimg.width);
    ...
    class JpegIO : ImgIO
    {
        ...
        override Image readMem(in ubyte[] bytes, in ImageFormat format)
        {
            ...
            immutable rowStride = imageAlignedStride(ImageFormat.argb, width);
