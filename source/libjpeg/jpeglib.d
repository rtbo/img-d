...
alias JSAMPROW = JSAMPLE*; /* ptr to one image row of pixel samples. */
alias JSAMPARRAY = JSAMPROW*; /* ptr to some rows (a 2-D sample array) */
alias JSAMPIMAGE = JSAMPARRAY*; /* a 3-D sample array: top index is color */

alias JBLOCK = JCOEF[DCTSIZE2]; /* one block of coefficients */
alias JBLOCKROW = JBLOCK*; /* pointer to one row of coefficient blocks */
alias JBLOCKARRAY = JBLOCKROW*; /* a 2-D array of coefficient blocks */
alias JBLOCKIMAGE = JBLOCKARRAY*; /* a 3-D array of coefficient blocks */

alias JCOEFPTR = JCOEF*; /* useful in a couple of places */
...
{
    return jpeg_CreateCompress(cinfo, JPEG_LIB_VERSION, jpeg_compress_struct.sizeof);
}

auto jpeg_create_decompress(j_decompress_ptr cinfo)
{
    return jpeg_CreateDecompress(cinfo, JPEG_LIB_VERSION, jpeg_decompress_struct.sizeof);
}
...
JDIMENSION jpeg_write_scanlines(j_compress_ptr cinfo, in JSAMPARRAY scanlines, JDIMENSION num_lines);
